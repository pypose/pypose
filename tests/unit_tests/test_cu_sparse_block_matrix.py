...
from pypose.optim.sparse_block_matrix import (
    SparseBlockMatrix, sbm_to_bsr_cpu )
from pypose.optim.cu_sparse_block_matrix import sbm_to_cupy
...
class Test_cu_SparseBlockMatrix(unittest.TestCase):
    ...
    def test_sbm_to_cupy(self):
        print()
        show_delimeter('Test sbm to CuPy conversion. ')

        # Creawte the SparseblockMatrix.
        device = 'cuda'
        values = Test_cu_SparseBlockMatrix.values_raw.to(device)
        sbm = SparseBlockMatrix(Test_cu_SparseBlockMatrix.block_shape, dtype=torch.float32, device=device)
        sbm.create(shape_blocks=Test_cu_SparseBlockMatrix.shape_blocks, block_indices=Test_cu_SparseBlockMatrix.block_indices)
        sbm.set_block_storage(values, clone=False)

        # Convert to CuPy.
        cu_sm = sbm_to_cupy(sbm)
        ...
        # Compare.
        self.assertTrue( np.allclose( np_cu, np_bsm ), f'test_sbm_to_cupy failed. ' )

    def test_sbm_to_cupy_duplicate_block_index(self):
        print()
        show_delimeter('Test sbm to CuPy conversion with duplicated block index. ')

        indices = copy.deepcopy( Test_cu_SparseBlockMatrix.block_indices )
        indices[0][-1] = 0
        indices[1][-1] = 0
        ...
        values = Test_cu_SparseBlockMatrix.values_raw.to(device)
        sbm = SparseBlockMatrix(Test_cu_SparseBlockMatrix.block_shape, dtype=torch.float32, device=device)
        sbm.create(shape_blocks=Test_cu_SparseBlockMatrix.shape_blocks, block_indices=indices)
        sbm.set_block_storage(values, clone=False)
        ...
        # Compare.
        self.assertTrue( np.allclose( np_cu, np_bsm ), f'test_sbm_to_cupy_duplicate_block_index failed. ' )
