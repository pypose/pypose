...
class TestSpline:
    ...
    def test_chspline(self):
        ...
        num = points.shape[-2]
        po = interpoints.index_select(-2, torch.arange(0, int(num/0.1), 1/0.1, device=device).int())
        assert (points - po).sum()<1e-5
        # test multi points
        points = torch.randn(20, 3, device=device)
        interpoints = pp.chspline(points, interval=0.2)
        num = points.shape[-2]
        po = interpoints.index_select(-2, torch.arange(0, int(num/0.2), 1/0.2, device=device).int())
        assert (points - po).sum()<1e-5
        # test multi batches
        points = torch.randn(3,20,3, device=device)
        interpoints = pp.chspline(points, interval=0.25)
        num = points.shape[-2]
        po = interpoints.index_select(-2, torch.arange(0, int(num/0.25), 1/0.25,device=device).int())
        assert (points - po).sum()<1e-5
