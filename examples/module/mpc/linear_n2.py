...
class TrainMPC:
    def main(self, device='cpu'):
        ...
        dt = 1
        stepper = pp.utils.ReduceToBason(steps=1, verbose=True)
        ...
        def get_loss(x_init, _A, _B):
            ...
            mpc_expert = pp.module.MPCN(lti, expert['Q'], expert['p'], T, stepper=stepper)
            x_true, u_true, cost_true = mpc_expert.forward(dt, x_init)

            lti_ = pp.module.LTI(_A, _B, C, D, c1, c2)
            mpc_agent = pp.module.MPCN(lti_, expert['Q'], expert['p'], T, stepper=stepper)
