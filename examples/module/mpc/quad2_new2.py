...
class Quad(pp.module.NLS):
    ...
    def __init__(self, dt, J, mass=7.4270, gravity=9.81):
        super().__init__()
        self._tau = dt
        self._J = J
        self._m = mass
...
def visualize(system, traj, controls):
    ...
    axs[0].plot(np.arange(traj.size(0)) * system._tau, np.radians(traj[:, 0]), linewidth=3.5)
    axs[1].plot(np.arange(traj.size(0)) * system._tau, np.radians(traj[:, 1]), linewidth=3.5)
    axs[2].plot(np.arange(traj.size(0)) * system._tau, np.radians(traj[:, 2]), linewidth=3.5)

    axs[0].set_title('roll', fontsize=28)
    axs[1].set_title('pitch', fontsize=28)
    axs[2].set_title('yaw', fontsize=28)

    axs[0].set_xlabel('Time(s)', fontsize=28)
    axs[1].set_xlabel('Time(s)', fontsize=28)
    axs[2].set_xlabel('Time(s)', fontsize=28)

    axs[0].set_ylabel('radian(rad)', fontsize=26, labelpad=5)
    axs[1].set_ylabel('radian(rad)', fontsize=26, labelpad=5)
    axs[2].set_ylabel('radian(rad)', fontsize=26, labelpad=5)
...
if __name__ == '__main__':
    ...
    Q[...,0,0], Q[...,1,1], Q[...,2,2], Q[...,8,8] = 10, 0.1, 0.1, 100
