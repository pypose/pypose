...
if __name__ == "__main__":
    ...
    exp = dict(
        ...
        p=torch.tensor(
            [0.6336, -0.2203, -0.1395, -0.7664, 0.8874, 0.8153], device=args.device
        ).tile(n_batch, T, 1),
        A=torch.tensor(
            [
                [1.1267, -0.0441, -0.0279],
                [-0.1533, 1.1775, 0.1631],
                [0.1618, 0.1238, 0.9489],
            ],
            device=args.device,
        ),
        B=torch.tensor(
            [
                [0.4567, 0.7805, 0.0319],
                [-0.5938, -0.5724, 0.0422],
                [-0.1804, -0.2535, 1.7218],
            ],
            device=args.device,
        ),
    )
    ...
    A = torch.tensor(
        [
            [1.2082, -0.1587, -0.3358],
            [0.2137, 0.8831, -0.1797],
            [0.1807, 0.2676, 0.7561],
        ],
        device=args.device,
        requires_grad=True,
    )
    B = torch.tensor(
        [
            [-0.3033, -0.4966, 0.0820],
            [-0.9567, 1.0006, -0.9712],
            [0.0227, -0.6663, 0.2731],
        ],
        device=args.device,
        requires_grad=True,
    )
    ...
    def get_loss(x_init, _A, _B):
        ...
        mpc_exp = pp.module.MPC(
            lti, exp["Q"], exp["p"], T, stepper=stepper_exp
        )  # expert
        ...
        mpc_agt = pp.module.MPC(
            lti_, exp["Q"], exp["p"], T, stepper=stepper_agt
        )  # agent
