...
def main():
    ...
    x_init = torch.tensor([[3, 0, torch.pi, 0]], device=device)
    current_x[...,0,:] = x_init
    ...
    mpc_expert = pp.module.MPC(expert_cartPoleSolver, T, step=5).to(device)
    ...
    def get_loss(_len, _m_cart, _m_pole):
        ...
        mpc_agent = pp.module.MPC(agent_cartPoleSolver, T, step=5).to(device)
