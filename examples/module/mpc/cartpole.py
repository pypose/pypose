...
def main():
    ...
    mpc_expert = pp.module.NLMPC(expert_cartPoleSolver, T, step=15).to(device)
    x_true, u_true, cost_true = mpc_expert(x_init, current_x, current_u, expert['Q'], expert['p'], time)
    ...
    def get_loss(_len, _m_cart, _m_pole):
        ...
        mpc_agent = pp.module.NLMPC(agent_cartPoleSolver, T, step=15).to(device)
        x_pred, u_pred, cost_pred = mpc_agent(x_init, current_x, current_u, expert['Q'], expert['p'], time)
