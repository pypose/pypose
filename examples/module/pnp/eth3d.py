...
import py7zr
import torchdata
from torchdata.datapipes.iter import FileOpener, HttpReader, IterableWrapper, IterDataPipe, Mapper, Zipper
...
def load_camera(file):
    ...
    camera_data = np.loadtxt(file, skiprows=3, usecols=(2, 3, 4, 5, 6, 7), ndmin=2)  # WIDTH HEIGHT PARAMS[fx, fy, cx, cy,]
    camera_dict = {k.item(): build_intrinsic(*(v[2:])) for k, v in zip(camera_ids, camera_data)}
    return camera_dict
...
def load_points(file):
    ...
    point3d_dict = {k.item(): (xyz, rgb) for k, xyz, rgb in zip(point3d_ids, point3d_xyz, point3d_rgb)}
    return point3d_dict
...
def load_image(data):
    ...
    for i in range(0, len(image), 2):  # for each image
        ...
        image_points = np.fromstring(image[i + 1], dtype=np.float32, sep=' ').reshape(-1, 3)[:, :2]  # (N, 2)
        ...
        batched_points2d, batched_points_ids, batched_points_xyz = batches_of_2d_points(image_points, point_ids, point3d_xyz_dict, num_points, batch_size)

        # tile rot and t to batch_size
        batched_rot = np.tile(rot[None], (batch_size, 1, 1))
        batched_t = np.tile(t[None], (batch_size, 1))
        batched_camera_intrinsics = np.tile(camera_data_dict[camera_id][None], (batch_size, 1, 1))
