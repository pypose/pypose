...
def msqrt(input, method='mpa'):
    ...
    Note:
        - ``input`` has to be a (:math:`b\times n \times n`) tensor.
        - ``out`` will be a (:math:`b\times n \times n`) tensor.
    Example1:
        >>> matrix = torch.tensor([[100, 0], [0, 100]], dtype=torch.float).unsqueeze(0)
        >>> out = pp.msqrt(matrix)
        >>> torch.sum(torch.bmm(out, out) - matrix)
        tensor(-3.0518e-05)
    '''
...
class Msqrt:
    ...
    def taylor_torch(self, fun, point, degree):
        r'''
        #compute taylor series of fun
        '''
        taylor_series = [fun.data]
        for i in torch.arange(0, degree, device=point.device):
            grads = grad(fun, point, create_graph=True)[0]
            data = grads.data / torch.lgamma(i + 2).exp()
            taylor_series.append(data)
            fun = grads

        return taylor_series

    def pade_torch(self, a, L, M):
        r'''
        compute Pade approximation

        Note:
            code is refactor from https://github.com/mpmath/mpmath/blob/1258e33e1618a6c537947775a03a7746362c4cee/mpmath/calculus/differentiation.py
        '''
        ...
        v = - torch.tensor(a[(L + 1):(L + M + 1)]).reshape(-1, 1)
        LU, pivots = torch.linalg.lu_factor(A)
        x = torch.lu_solve(v, LU, pivots)
        q = torch.cat([torch.ones(1).reshape(-1, 1), x]).reshape(-1)
        # compute p
        p = torch.zeros(L + 1)
