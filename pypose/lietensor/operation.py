...
class SO3_Log(torch.autograd.Function):
  ...
  def backward(ctx, grad_output):
    ...
    grad_input[..., :-1] = (grad_output.unsqueeze(-2) @ Jl_inv).squeeze(-2)
    return grad_input
...
class SE3_Log(torch.autograd.Function):
  ...
  def backward(ctx, grad_output):
    ...
    grad_input = torch.zeros(output.shape[:-1]+(7,), device=output.device, dtype=output.dtype)
    grad_input[..., :-1] = (grad_output.unsqueeze(-2) @ Jl_inv).squeeze(-2)
    return grad_input
...
class RxSO3_Log(torch.autograd.Function):
  ...
  def backward(ctx, grad_output):
    ...
    grad_input = torch.zeros(output.shape[:-1]+(5,), device=output.device, dtype=output.dtype)
    grad_input[..., :-1] = (grad_output.unsqueeze(-2) @ Jl_inv).squeeze(-2)
    return grad_input
...
class Sim3_Log(torch.autograd.Function):
  ...
  def backward(ctx, grad_output):
    ...
    grad_input = torch.zeros(output.shape[:-1]+(8,), device=output.device, dtype=output.dtype)
    grad_input[..., :-1] = (grad_output.unsqueeze(-2) @ Jl_inv).squeeze(-2)
    return grad_input