...
def make_coo_indices_and_dims_from_hybrid(hybrid):
    ...
    indices_ori = hybrid.indices().unsqueeze(-1)  # (b_dim, num_b, 1)
    indices_rp = indices_ori.expand(-1, -1, numel_b)  # (b_dim, num_b, numel_b)
...
def coo_2_hybrid(coo, proxy, dense_proxy_thres: int=DENSE_SAFE_PROXY_THRES):
    ...
    assert all(coo.shape[i] % shape_p[i] == 0 for i in range(proxy.dim())), \
        f'coo and s_dim are not compatible: coo.shape = {coo.shape}, s_dim = {shape_p}. '
