...
class RANSAC(nn.Module):
    def __init__(self, method = 'ransac'):
        super().__init__()
        self.supported_methods = ["ransac", "ransac_adapt"]

        if method == 'ransac':
            self.func = self.ransac
        elif method == 'ransac_adapt':
            self.func = self.ransac_adapt
        else:
            raise NotImplementedError(f"{method} is unknown. Try one of {self.supported_methods}")
    ...
    def ransac_adapt(data:dict,fit_model,check_model,confidence = 0.999,num_of_select = 8, threshold = 1):
        ...
        """
        assert isinstance(confidence,float) == True, "The parameter 'terminate' should be a float that greater than 0 and less than 1!"
        assert confidence <= 1 and confidence >= 0, "The parameter 'terminate' should be a float that greater than 0 and less than 1!"
        ...
        while iters < k:
            ...
            samples = torch.randint(0,h_int,(num_of_select,)) 
            # avoid selecting a sample multiple times
            if len(torch.unique(samples))!= num_of_select:
                continue

            # run fit model
            M = fit_model(data,torch.unsqueeze(samples,dim = 0))
    ...
    def forward(self,data, fit_model, check_model, terminate, num_of_select = 8, threshold = 1):
        ...
        M, mask = self.func(data, fit_model, check_model, terminate, num_of_select, threshold)
