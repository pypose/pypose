...
class LQRLS(nn.Module):
    ...
    **iterative LQR**. A discrete-time non-linear system can be described as:
    ...
    def lqr_forward(self, x_init, K, k, u_lower=None, u_upper=None, du=None):
        ...
        new_cost = torch.zeros(self.n_batch, **self.dargs)
        old_cost = torch.zeros(self.n_batch, **self.dargs)
        x = torch.zeros(self.n_batch + (self.T+1, ns), **self.dargs)
        xt = x[..., 0, :] = x_init
        max_linesearch_iters = 3
        i = 0
        alpha = 1.0

        while i < max_linesearch_iters:
            i = i + 1
            #print(i)
            ...
            for t in range(self.T):
                ...
                delta_u[..., t, :] = bmv(Kt, delta_xt) + alpha * kt
                u[...,t,:] = ut = delta_u[..., t, :] + self.u_traj[...,t,:]
                xut = torch.cat((xt, ut), dim=-1)
                old_xut = torch.cat((self.x_traj[...,t,:], self.u_traj[...,t,:]), dim=-1)
                x[...,t+1,:] = xt = self.system(xt, ut)[0]
                new_cost += 0.5 * bvmv(xut, self.Q[...,t,:,:], xut) + vecdot(xut, self.p[...,t,:])
                old_cost += 0.5 * bvmv(old_xut, self.Q[...,t,:,:], old_xut) + vecdot(old_xut, self.p[...,t,:])

            if not new_cost > old_cost:
                break
            else:
                alpha = 0.5 * alpha
                self.x_traj = x
