...
class LQRLS(nn.Module):
    ...
    def lqr_forward(self, x_init, current_x, current_u, K, k):
        ...
        current_cost = torch.zeros(self.n_batch, dtype=self.p.dtype, device=self.p.device)
        ...
        for i in range(max_linesearch_iters):
            ...
            for t in range(self.T):
                Kt, kt = K[...,t,:,:], k[...,t,:]
                delta_u[..., t, :] = delta_ut = bmv(Kt, delta_xt) + alpha * kt
                ...
                current_xut = torch.cat((current_x[...,t,:], current_u[...,t,:]), dim=-1)
                current_cost = current_cost + 0.5 * bvmv(current_xut, self.Q[...,t,:,:], current_xut) + (current_xut * self.p[...,t,:]).sum(-1)
