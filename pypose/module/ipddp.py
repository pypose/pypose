    ...
    def forwardpasscompact(self):
        ...
        for step in range(steplist.shape[0]): # line search
            ...
            if (failed):
                ...
                if (self.infeas):
                    logcost = - self.mu * ynew.log().sum(-1).sum(-1)
                    cnew = self.c_fn(xnew[...,:-1,:], unew)
                    err = torch.linalg.vector_norm(cnew + ynew, 1).sum(-1)
