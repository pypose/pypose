...
class TestIPDDP:
    ...
    def test_ipddp_linear(self, device='cpu'):
        ...
        gx = torch.tile(torch.zeros( 2*n_ctrl, n_state, device=device), (n_batch, 1, 1))
        gu = torch.tile(torch.vstack( (torch.eye(n_ctrl, n_ctrl, device=device), - torch.eye(n_ctrl, n_ctrl, device=device)) ), (n_batch, 1, 1))
        g = torch.tile(torch.hstack( (-0.25 * torch.ones(1, n_ctrl, device=device), -0.25 * torch.ones(1, n_ctrl, device=device)) ), (n_batch, 1, 1))
        lincon = pp.module.LinCon(gx, gu, g)
        ipddp = pp.module.ddpOptimizer(lti, lincon, Q, p, T).to(device)
        x, u, cost, cons = ipddp(x_init)
