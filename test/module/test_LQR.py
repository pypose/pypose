...
def test_lqr_linear():
    ...
    F = torch.cat((A[0,:,:], B[0,:,:]), dim = 1)
    F = torch.tile(F, (T, 1, 1))
    f = torch.tile(c1, (T, 1, 1))

    tau_cp, objs_cp = lqr_cp(Q[0,:,:,:], p[0,:,:], F, f[:,0], x_init[0], T, n_state, n_ctrl)
    tau_cp = tau_cp.T
    x_cp = tau_cp[:,:n_state]
    u_cp = tau_cp[:,n_state:]
    ...
    x_lqr, u_lqr, objs_lqr, tau = LQRDP(x_init, Q, p)
...
def test_lqr_ltv_random():
    ...
    c1 = torch.randn(n_batch, T, n_state)
    c2 = torch.tile(torch.zeros(n_state), (n_batch, T, 1))
    x_init = torch.randn(n_batch, n_state)
